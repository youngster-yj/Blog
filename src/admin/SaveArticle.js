...
class SaveArticle extends Component {
    ...
    saveArticle() {
        if (this.condition()) {
            ...
            let articleTitle = this.state.articleTitle //文章标题
                ...
                fetch(servicePath.uploadsaveBlog, {
                    method: 'POST',
                    headers: {
    ...
    getArticleById(id) {//接受从修改按钮发过来得值 查询数据库并填充页面      
        //数据请求 填充页面
        fetch(servicePath.getSaveByName+'?articleTitle='+id, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/x-www-form-urlencoded',
            },
                ...
                <Row gutter={5}>
                    ...
                    <Col span={6}>
                        <Row>
                            <Col span={24}>
                                <Col span={12}>
                                    <Button size='large' onClick={() => this.saveArticle()} disabled={this.state.disabled}>更新存储</Button>
                                </Col>
                                <Col span={12}>
                                    <Button type='primary' size='large' onClick={() => this.postArticle()}>{'发布文章'}</Button>
                                </Col>
                            </Col>
                            <Col span={24}>
                                <br />
                                <TextArea rows={4} placeholder='文章简介' onChange={(e) => { this.changeIntroduce(e.target.value) }} value={this.state.introducemd}></TextArea>
                                <br /><br />
