...
import { Row, Col, List, Icon, Affix, BackTop,message,Spin, Tag,Input } from 'antd'
  ...
  componentWillMount(){
    ...
    document.addEventListener('visibilitychange',function(){
      ...
      document.title = '嘤嘤嘤，你回来了啊(ಥ _ ಥ)';
        setTimeout(()=>{
          document.title = '实战页 | Youngster_yj的个人博客'
        },3000)
      }
      }
      );
    ...
    document.title = '实战页 | Youngster_yj的个人博客'
    fetch(servicePath.getArticleList+'?name=实战分享', {
      method: 'GET',
      headers: {
    ...
    return ( 
      ...
      <Row className='comm-main' type='flex' justify='center' style={{paddingTop:'3.2rem'}}>
        ...
        <Spin tip='加载中...' spinning={this.state.isLoading}>
          <List
            header={<Row><Col xs={12} sm={14} md={15} lg={17} xl={17}><div style={{ fontWeight: 'bold', paddingLeft: 20 ,lineHeight: '32px'}}>实战积累 <span style={{color: 'red'}}>{this.state.getblog.length}</span> 篇</div></Col>
            <Col xs={11} sm={9} md={8} lg={6} xl={6}><Search value={this.state.searchValue} placeholder="搜索实战内容" onChange={(e)=>{this.searchInput(e)}} /></Col>
...
<LazyLoad height={200} offset={-200}>
              ...
              <span><Icon type='logout' style={{color:'#ff7d00'}}/> {item.sourceType==undefined?'实战分享':item.sourceType}</span>
